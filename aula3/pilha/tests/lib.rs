use std::vec::Vec; // Importa o módulo Vec para armazenar os elementos da pilha

// Definição da estrutura de Pilha (Stack)
pub struct Stack<T> {
    // A implementação será adicionada após criar os testes
    elementos: Vec<T>, // Armazena os elementos da pilha
    capacidade_maxima: Option<usize>, // Capacidade máxima
}
